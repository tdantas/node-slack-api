 ...
 this.timeout(90000);
  ...
  it('initialize with a token', function(done) {
    assert.doesNotThrow(function() { 
      slackClient = Client('MY_TOKEN', { timeout: 500 }); 
    });
    done();
  });

  it('fails when request is bigger than timeout', function(done){
    var body = Fixture.UsersInfo;

    scope = mockRequest('/api/users.info?token=MY_TOKEN&user=U023BECGF', body, 1500);

    slackClient.usersInfo({ user: 'U023BECGF' }, function(err) {
     assert.ok(err);
     done();
    });
